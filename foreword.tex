...
The Hazard Pointers library is described in P2530R2 (``Hazard Pointers
in C++26'').
...
\item	\ref{saferecl.rcu.domain}: Add \tcode{try_lock()} in order
	...
	deadlock).
\item	\ref{saferecl.rcu.domain.lock}p2: Change ``the function
	\tcode{lock}'' to just ``\tcode{lock}''.
\item	\ref{saferecl.rcu.domain.unlock}p1 and
	\ref{saferecl.rcu.domain.unlock}p3: Change ``the function
	\tcode{unlock}'' to just ``\tcode{unlock}''.
\item	\ref{saferecl.rcu.default.domain}p1: Change ``the default object''
	to ``a static-duration object''.
\item	Add a note following \ref{saferecl.rcu.barrier} stating that
	neither of \tcode{rcu_synchronize} and \tcode{rcu_barrier}
	imply the other.
\item	\ref{saferecl.rcu.retire}: Refactor the mandates, preconditions
	(AKA expects), and effects clauses.
\item	\ref{saferecl.rcu.retire}p4: Rely on common Library wording
	to avoid special wording for \tcode{bad_alloc} subclasses.
\end{itemize}
...
of P2530R2 (``Hazard Pointers in C++26'') that was carried
