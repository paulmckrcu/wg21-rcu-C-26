...
\item	\ref{saferecl.rcu.domain}: Add \tcode{try_lock()} in order
	...
	deadlock).
\item	\ref{saferecl.rcu.domain.members}p2 and
	\ref{saferecl.rcu.domain.members}p5:
	Change ``the function
	\tcode{lock}'' to just ``\tcode{lock}''.
\item	\ref{saferecl.rcu.domain.members}p7: Change ``the function
	\tcode{unlock}'' to just ``\tcode{unlock}''.
\item	\ref{saferecl.rcu.domain.nonmembers}p1: Change ``the default object''
	to ``a static-duration object''.
\item	Add a note following \ref{saferecl.rcu.updates}p4 stating that
	neither of \tcode{rcu_synchronize} and \tcode{rcu_barrier}
	imply the other.
\item	\ref{saferecl.rcu.updates}p5-7: Refactor the mandates, preconditions
	(AKA expects), and effects clauses for \tcode{rcu_retire}.
\item	\ref{saferecl.rcu.updates}p8: Rely on common Library wording
	to avoid special \tcode{rcu_retire} wording for \tcode{bad_alloc}
	subclasses.
\end{itemize}
