...
\tcode{std::rcu_obj_base<T>} and use the \tcode{std::rcu_retire()} free function.
...
% Google's would make eleven, but approval pending.
...
% Google uses an internally developed C++ RCU implementation that inspired
...
% Awaiting approval from Geoffrey Romer.
